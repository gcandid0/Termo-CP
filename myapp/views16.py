...
# Importando os novos formulários da Amônia (S = cte)
from .forms import ScteAmonia, ScteAmonia2, ScteAmonia3
...
def homepage_view16(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view16(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_16')
    else:
        form = ScteAmonia()

    return render(request, 'amonia-scte-1.html', {'form': form})

###############################################################################

def ask_known2_view16(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = ScteAmonia2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_16')
    else:
        form = ScteAmonia2(excluded_properties=excluded_properties)

    return render(request, 'amonia-scte-2.html', {'form': form})

###############################################################################

def ask_known3_view16(request):
    if request.method == 'POST':
        form = ScteAmonia3(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_16')
    else:
        form = ScteAmonia3()

    return render(request, 'amonia-scte-3.html', {'form': form})
...
def process_values_view16(request):
    ...
    try:
        ...
        return render(request, 'results-amonia-scte-4.html', {
            'fase': fase,
