...
from .forms import PcteAmonia, PcteAmonia2, PcteAmonia3, PcteAmonia4
...
def homepage_view15(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view15(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_15')
    else:
        form = PcteAmonia()

    return render(request, 'amonia/amonia-pcte-1.html', {'form': form})

###############################################################################

def ask_known2_view15(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = PcteAmonia2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_15')
    else:
        form = PcteAmonia2(excluded_properties=excluded_properties)

    return render(request, 'amonia/amonia-pcte-2.html', {'form': form})

###############################################################################

def ask_known3_view15(request):
    if request.method == 'POST':
        form = PcteAmonia3(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known4_15')
    else:
        form = PcteAmonia3()

    return render(request, 'amonia/amonia-pcte-3.html', {'form': form})


###############################################################################

def ask_known4_view15(request):
    if request.method == 'POST':
        form = PcteAmonia4(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_15')
    else:
        form = PcteAmonia4()

    return render(request, 'amonia/amonia-pcte-4.html', {'form': form})
...
def process_values_view15(request):
    ...
    try:
        ...
        if estados.lista_estados:
            ...
            # Instanciando com opt=2 (Amônia) em vez de 1 (Água)
        ...
        if third_property_choice == 8:
            ...
            # Calculando para a Amônia (opt=2) no estado 2
            h = subs_cls(2, 1, 4, pressao, entalpia_esp3)
        else:
            # Calculando para a Amônia (opt=2) no estado 2
            h = subs_cls(2, 1, third_property_choice, pressao, third_value_input)
        ...
        try:
            ...
            h_sat_l = subs_cls(2, 1, 6, pressao, 0)
            h_sat_v = subs_cls(2, 1, 6, pressao, 100)
        ...
        for val_atual in valores_iteracao:
            try:
                # O '2, 1, 4' significa: Substância 2 (Amônia), Prop Conhecida 1 (Pressão), Iterando Prop 4 (Entalpia)
                h_micro = subs_cls(2, 1, 4, pressao, val_atual)
        ...
        # Garante que salva no objeto estados também, se existir suporte
        if hasattr(estados, 'pontos_grafico'):
        ...
        return render(request, 'amonia/results-amonia-pcte-5.html', {
            'fase': fase,
...
def error_value_view15(request):
