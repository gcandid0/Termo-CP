...
from .forms import PropertyFormAmoniaEst1, SecondPropertyFormAmoniaEst1
...
def ask_known1_view14(request):
    if request.method == 'POST':
        form = PropertyFormAmoniaEst1(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known2_14')
    else:
        form = PropertyFormAmoniaEst1()

    return render(request, 'amonia/amonia-propriedade-1.html', {'form': form})

###############################################################################

def ask_known2_view14(request):
    if request.method == 'POST':
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormAmoniaEst1(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            property_choice = form.cleaned_data['property_choice']
            value_input = form.cleaned_data['value_input']
            request.session['second_property_choice'] = property_choice
            request.session['second_value_input'] = value_input
            return redirect('process_values_14')
    else:
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormAmoniaEst1(excluded_properties=excluded_properties)

    return render(request, 'amonia/amonia-propriedade-2.html', {'form': form})
                ...
                if self.phase == 1:
                    ...
                    if self.index1 != 1 and self.index2 != 1:
                        if self.n == 1: self.looking_for_not_p_in_lc()
                            ...
                            # Removido o redirect problemático. Lança erro.
...
def process_values_view14(request):
    ...
    try:
        ...
        # Instanciando a nova classe subs_cls. O valor '2' indica Amônia
        ...
        return render(request, 'amonia/results-amonia-propriedade-3.html', {
            'fase': fase,
...
def error_value_view14(request):
