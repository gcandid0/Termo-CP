...
# Importando os formulários específicos da Amônia para Volume Constante
from .forms import vcteAmonia, vcteAmonia2, vcteAmonia3, vcteAmonia4
...
def homepage_view17(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view17(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_17')
    else:
        form = vcteAmonia()

    return render(request, 'amonia/amonia-vcte-1.html', {'form': form})

###############################################################################

def ask_known2_view17(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = vcteAmonia2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_17')
    else:
        form = vcteAmonia2(excluded_properties=excluded_properties)

    return render(request, 'amonia/amonia-vcte-2.html', {'form': form})

###############################################################################

def ask_known3_view17(request):
    if request.method == 'POST':
        form = vcteAmonia3(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known4_17')
    else:
        form = vcteAmonia3()

    return render(request, 'amonia/amonia-vcte-3.html', {'form': form})

###############################################################################

def ask_known4_view17(request):
    if request.method == 'POST':
        form = vcteAmonia4(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_17')
    else:
        form = vcteAmonia4()

    return render(request, 'amonia/amonia-vcte-4.html', {'form': form})
...
def process_values_view17(request):
    ...
    try:
        ...
        if estados.lista_estados:
            ...
            # Inicializa a Amônia (opt=2) com as entradas da Sessão
        ...
        # O formulário vcteAmonia4 fornece:
        ...
        h = subs_cls(2, third_property_choice, 2, third_value_input, volume_esp)
        ...
        # ==========================================================
        # GERAÇÃO DE MICROPROCESSOS PARA O DIAGRAMA (ALTA PRECISÃO - VOLUME CONSTANTE)
        # ==========================================================
        pontos_grafico = request.session.get('pontos_grafico', [])
        
        # O SEGREDO DO ISOVOLUMÉTRICO: Iterar pela Entalpia ou Energia Interna para desenhar sem quebrar
        val_inicial = entalpia_esp
        val_final = entalpia_esp2

        # 1. CAPTURA DOS PONTOS CRÍTICOS (Fronteiras do Domo via Entalpia a V=cte)
        pontos_criticos = []
        try:
            # Para Amônia, usamos subs_cls com opt=2
            h_sat_l = subs_cls(2, 2, 6, volume_esp, 0)
            h_sat_v = subs_cls(2, 2, 6, volume_esp, 100)
        ...
        # Processamento da curva: Forçamos o índice 4 (Entalpia) e fixamos o 2 (Volume)
        for val_atual in valores_iteracao:
            try:
                # O '2, 2, 4' significa: Substância 2 (Amônia), Prop Conhecida 2 (Volume), Iterando Prop 4 (Entalpia)
                h_micro = subs_cls(2, 2, 4, volume_esp, val_atual)
                
                # Coleta com segurança
                T_micro = h_micro.results[2][0][3]
                P_micro = h_micro.results[2][1][3]
                s_micro = h_micro.results[2][5][3]

                if T_micro != 0 and P_micro != 0:
                    ramo_atual.append({
                        'T': round(T_micro, 2),
                        'P': round(P_micro, 2),
                        'v': round(volume_esp, 6),
        ...
        return render(request, 'amonia/results-amonia-vcte-5.html', {
            'fase': fase,
