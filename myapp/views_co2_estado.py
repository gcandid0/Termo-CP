# -*- coding: utf-8 -*-
"""
Created on Tue Apr 14 18:14:36 2026

@author: gabri
"""

from django.shortcuts import render, redirect
from .forms import PropertyFormCO2Est1, SecondPropertyFormCO2Est1
from django.core.exceptions import ValidationError
from . import tabelas_termoprop as tbs

def processos_view18(request):
    return render(request, 'processos4.html')

###############################################################################

def ask_known1_view18(request):
    if request.method == 'POST':
        form = PropertyFormCO2Est1(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known2_18')
    else:
        form = PropertyFormCO2Est1()

    return render(request, 'co2/co2-estado-1.html', {'form': form})

###############################################################################

def ask_known2_view18(request):
    if request.method == 'POST':
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormCO2Est1(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            property_choice = form.cleaned_data['property_choice']
            value_input = form.cleaned_data['value_input']
            request.session['second_property_choice'] = property_choice
            request.session['second_value_input'] = value_input
            return redirect('process_values_18')
    else:
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormCO2Est1(excluded_properties=excluded_properties)

    return render(request, 'co2/co2-estado-2.html', {'form': form})
...
def process_values_view18(request):
    ...
    try:
        ...
        return render(request, 'co2/results-co2-estado-3.html', {
            'fase': fase,
...
def error_value_view18(request):
