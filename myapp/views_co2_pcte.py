...
from .forms import PcteCO2, PcteCO22, PcteCO23, PcteCO24
from django.core.exceptions import ValidationError
import json
import math # Importação necessária para os cálculos de microprocessos
...
def ask_known1_view20(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_20')
    else:
        form = PcteCO2()

    return render(request, 'co2/co2-pcte-1.html', {'form': form})

###############################################################################

def ask_known2_view20(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = PcteCO22(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_20')
    else:
        form = PcteCO22(excluded_properties=excluded_properties)

    return render(request, 'co2/co2-pcte-2.html', {'form': form})

###############################################################################

def ask_known3_view20(request):
    if request.method == 'POST':
        form = PcteCO23(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known4_20')
    else:
        form = PcteCO23()

    return render(request, 'co2/co2-pcte-3.html', {'form': form})

###############################################################################

def ask_known4_view20(request):
    if request.method == 'POST':
        form = PcteCO24(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_20')
    else:
        form = PcteCO24()

    return render(request, 'co2/co2-pcte-4.html', {'form': form})
...
def process_values_view20(request):
    ...
    try:
        ...
        if third_property_choice == 8:
            ...
            h = subs_cls(3, 1, 4, pressao, entalpia_esp3)
        else:

            h = subs_cls(3, 1, third_property_choice, pressao, third_value_input)
        ...
        pontos_criticos = []
        try:
            # Opt 3 (CO2), Prop Conhecida 1 (P), Prop a determinar 6 (Título)
            h_sat_l = subs_cls(3, 1, 6, pressao, 0)
            h_sat_v = subs_cls(3, 1, 6, pressao, 100)
        ...
        # Processamento da curva (Forçando o índice 4: Entalpia)
        for val_atual in valores_iteracao:
            try:
                # Opt 3 (CO2), Prop Conhecida 1 (P), Iterando Prop 4 (Entalpia)
                h_micro = subs_cls(3, 1, 4, pressao, val_atual)
        ...
        return render(request, 'co2/results-co2-pcte-5.html', {
            'fase': fase,
            ...
            'Trabalho': trabalho,
            'teste3': teste3,
            'pontos_grafico_json': json.dumps(pontos_grafico) # Passando os pontos desenhados no context
        })
...
def error_value_view20(request):
