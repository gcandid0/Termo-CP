...
def ask_known1_view19(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_19')
    else:
        form = PropertyFormCO2Est1()

    return render(request, 'co2/co2-propriedade-1.html', {'form': form})

###############################################################################

def ask_known2_view19(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('process_values_19')
    else:
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormCO2Est1(excluded_properties=excluded_properties)

    return render(request, 'co2/co2-propriedade-2.html', {'form': form})
...
def process_values_view19(request):
    ...
    try:
        ...
        return render(request, 'co2/results-co2-propriedade-3.html', {
            'fase': fase,
...
def error_value_view19(request):
