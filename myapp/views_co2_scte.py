...
from .forms import ScteCO2, ScteCO22, ScteCO23
from django.core.exceptions import ValidationError
from . import tabelas_termoprop as tbs
from . import estados as std
import json
import math # Importação necessária para a escala logarítmica
...
def homepage_view21(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view21(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_21')
    else:
        form = ScteCO2()

    return render(request, 'co2/co2-scte-1.html', {'form': form})

###############################################################################

def ask_known2_view21(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = ScteCO22(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_21')
    else:
        form = ScteCO22(excluded_properties=excluded_properties)

    return render(request, 'co2/co2-scte-2.html', {'form': form})

###############################################################################

def ask_known3_view21(request):
    if request.method == 'POST':
        form = ScteCO23(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_21')
    else:
        form = ScteCO23()

    return render(request, 'co2/co2-scte-3.html', {'form': form})
...
def process_values_view21(request):
    ...
    try:
        ...
        h = subs_cls(3, third_property_choice, 5, third_value_input, entropia_esp)
        ...
        if val_final != val_inicial:
            for val_atual in valores_iteracao:
                # Roteamento dinâmico baseado na variável e na propriedade fixada (s)
                ...
                try:
                    # ATENÇÃO: CO2 exige opt=3 na classe subs_cls
                    h_micro = subs_cls(3, idx1, idx2, val1, val2)
        ...
        return render(request, 'co2/results-co2-scte-4.html', {
            'fase': fase,
