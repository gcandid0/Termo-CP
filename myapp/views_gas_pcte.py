...
from .forms import (
    ...
    Prop1PCte8, Prop1PCte8_2, Prop2PCte8, TvizGasIdeal8
)
...
def ask_known1_view8(request):
    ...
    return render(request, 'gas/ask_known1_8.html', {'form': form})
...
def ask_known2_view8(request):
    ...
    return render(request, 'gas/ask_known2_8.html', {'form': form})
...
def ask_known3_view8(request):
    ...
    if request.method == 'POST':
        ...
        form = Prop1PCte8()
    return render(request, 'gas/ask_known3_8.html', {'form': form})
...
def ask_known4_view8(request):
    ...
    if request.method == 'POST':
        form = Prop1PCte8_2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
        ...
        form = Prop1PCte8_2(excluded_properties=excluded_properties)
    return render(request, 'gas/ask_known4_8.html', {'form': form})
...
def ask_known5_view8(request):
    if request.method == 'POST':
        ...
        form = Prop2PCte8()
    return render(request, 'gas/ask_known5_8.html', {'form': form})
...
def ask_known6_view8(request):
    if request.method == 'POST':
        ...
        form = TvizGasIdeal8()
    return render(request, 'gas/ask_known6_8.html', {'form': form})
...
def process_values_view8(request):
    try:
        ...
        # (isotérmico, isocórico, politrópico, etc.), fazendo o relatório final
        ...
        # =====================================
        # 2️⃣ Determinação das Constantes
        # =====================================
        ...
        if gas.lista_gas:
            # Usa SEMPRE o último estado salvo
            ultimo = gas.lista_gas[-1]

            # Estrutura: [Cv, Cp, R, K, p, T_C, v]
            p1 = float(ultimo[4])
            T1 = to_kelvin(float(ultimo[5])) # Kelvin
            v1 = float(ultimo[6])
        else:
            ...
            if T1 is None or p1 is None or v1 is None:
                return redirect('error_type8')
        ...
        p2 = p1  # PRESSÃO CONSTANTE
        Q12_input = None

        if prop2_idx == 0:  # T fornecido (em °C, converte para K)
            T2 = to_kelvin(prop2_val)
            v2 = (R * T2) / p2

        elif prop2_idx == 2:  # v fornecido
            v2 = prop2_val
            T2 = (p2 * v2) / R # Resultado direto em Kelvin

        elif prop2_idx == 8:  # Q fornecido
            Q12_input = prop2_val
            # T1 já está em Kelvin, o aumento em Kelvin é igual ao aumento em Celsius
            T2 = T1 + (Q12_input / Cp0)
            v2 = (R * T2) / p2
        ...
        if Q12_input is not None:
            ...
            # T2 e T1 em Kelvin, Cp0 em kJ/kgK
            Q12 = Cp0 * (T2 - T1)

        Tviz_K = to_kelvin(Tviz_input)

        try:
            # Entropia calculada sempre com temperaturas em KELVIN
        ...
        if passo_T != 0:
            for i in range(1, num_passos):
                ...
                P_micro = p1  # Processo Isobárico
                v_micro = (R * T_micro) / P_micro
        ...
        # =====================================
        # 6️⃣ SALVAR NOVO ESTADO E GERAR CONTEXTO
        # =====================================

        T2_C = to_celsius(T2) # Converte de volta para salvar na lista

        novo_estado = [
            float(Cv0),
            float(Cp0),
            float(R),
            float(K) if K else 0.0,
            float(p2),
            float(T2_C),
            float(v2)
        ]
        ...
        context = {
            ...
            'K': rd(K),
            'p': rd(p1),
            'T1': rd(to_celsius(T1)), # Exibe em °C
            'T2': rd(T2_C), # Exibe em °C
            'v1': rd(v1),
            'v2': rd(v2),
            'Q12': rd(Q12),
            'W12': rd(W12),
            'Sger': rd(Sger),
            'Tviz': rd(Tviz_input), # Exibe original em °C
            'teste': gas.lista_gas,
            'pontos_grafico_json': json.dumps(pontos_grafico), # Dados para o Chart.js
            'historico_processos_json': json.dumps(historico_processos) # Histórico p/ Relatório Final
        }

        return render(request, 'gas/results_8.html', context)
