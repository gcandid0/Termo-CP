...
def homepage_view40(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view40(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_40')
    else:
        form = PcteKelvin()

    return render(request, 'metano/mkelvin-pcte-1.html', {'form': form})

###############################################################################

def ask_known2_view40(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known3_40')
    else:
        form = PcteKelvin2(excluded_properties=excluded_properties)

    return render(request, 'metano/mkelvin-pcte-2.html', {'form': form})

###############################################################################

def ask_known3_view40(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known4_40')
    else:
        form = PcteKelvin3()

    return render(request, 'metano/mkelvin-pcte-3.html', {'form': form})


###############################################################################

def ask_known4_view40(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('process_values_40')
    else:
        form = PcteKelvin4()

    return render(request, 'metano/mkelvin-pcte-4.html', {'form': form})
...
def process_values_view40(request):
    ...
    try:
        ...
        if third_property_choice == 8:
            ...
            h = subs_cls(7, 1, 4, pressao, entalpia_esp3)
        else:
            h = subs_cls(7, 1, third_property_choice, pressao, third_value_input)
        ...
        pontos_criticos = []
        try:
            h_sat_l = subs_cls(7, 1, 6, pressao, 0)
            h_sat_v = subs_cls(7, 1, 6, pressao, 100)
        ...
        for val_atual in valores_iteracao:
            try:
                h_micro = subs_cls(7, 1, 4, pressao, val_atual)
        ...
        return render(request, 'metano/results-metano-pcte-5.html', {
            'fase': fase,
...
def error_value_view40(request):
