...
def homepage_view41(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view41(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_41')
    else:
        form = ScteKelvin()

    return render(request, 'metano/mkelvin-scte-1.html', {'form': form})

###############################################################################

def ask_known2_view41(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known3_41')
    else:
        form = ScteKelvin2(excluded_properties=excluded_properties)

    return render(request, 'metano/mkelvin-scte-2.html', {'form': form})

###############################################################################

def ask_known3_view41(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('process_values_41')
    else:
        form = ScteKelvin3()

    return render(request, 'metano/mkelvin-scte-3.html', {'form': form})
...
def process_values_view41(request):
    ...
    try:
        ...
        if third_property_choice == 7:
            ...
            h = subs_cls(7, 1, 4, pressao, energia_int3)
            ...
            h = subs_cls(7, third_property_choice, 5, third_value_input, entropia_esp)
        ...
        if cruzou_domo:
            ...
            for _ in range(60):
                ...
                try:
                    if is_isobaric:
                        h_mid = subs_cls(7, 1, 4, fixo_val, val_mid)
                    else:
                        h_mid = subs_cls(7, 1, 5, val_mid, fixo_val)
            ...
            try:
                if is_isobaric:
                    h_front_calc = subs_cls(7, 1, 4, fixo_val, val_front)
                else:
                    h_front_calc = subs_cls(7, 1, 5, val_front, fixo_val)
        ...
        return render(request, 'metano/results-metano-scte-4.html', {
            'fase': fase,
