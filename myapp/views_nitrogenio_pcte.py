...
from .forms import PcteKelvin, PcteKelvin2, PcteKelvin3, PcteKelvin4
...
def homepage_view35(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view35(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_35')
    else:
        form = PcteKelvin()

    return render(request, 'nitrogenio/nkelvin-pcte-1.html', {'form': form})

###############################################################################

def ask_known2_view35(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = PcteKelvin2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_35')
    else:
        form = PcteKelvin2(excluded_properties=excluded_properties)

    return render(request, 'nitrogenio/nkelvin-pcte-2.html', {'form': form})

###############################################################################

def ask_known3_view35(request):
    if request.method == 'POST':
        form = PcteKelvin3(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known4_35')
    else:
        form = PcteKelvin3()

    return render(request, 'nitrogenio/nkelvin-pcte-3.html', {'form': form})


###############################################################################

def ask_known4_view35(request):
    if request.method == 'POST':
        form = PcteKelvin4(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_35')
    else:
        form = PcteKelvin4()

    return render(request, 'nitrogenio/nkelvin-pcte-4.html', {'form': form})
...
def process_values_view35(request):
    ...
    try:
        ...
        if third_property_choice == 8:
            ...
            h = subs_cls(6, 1, 4, pressao, entalpia_esp3)
        else:
            h = subs_cls(6, 1, third_property_choice, pressao, third_value_input)
        ...
        pontos_criticos = []
        try:
            h_sat_l = subs_cls(6, 1, 6, pressao, 0)
            h_sat_v = subs_cls(6, 1, 6, pressao, 100)
        ...
        for val_atual in valores_iteracao:
            try:
                h_micro = subs_cls(6, 1, 4, pressao, val_atual)
        ...
        return render(request, 'nitrogenio/results-nitrogenio-pcte-5.html', {
            'fase': fase,
...
def error_value_view35(request):
