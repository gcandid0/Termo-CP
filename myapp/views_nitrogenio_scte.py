...
from .forms import ScteKelvin, ScteKelvin2, ScteKelvin3
...
def homepage_view36(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view36(request):
    estados.limpar_estados()  # limpa a lista
    if request.method == 'POST':
        form = ScteKelvin(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known2_36')
    else:
        form = ScteKelvin()

    return render(request, 'nitrogenio/nkelvin-scte-1.html', {'form': form})

###############################################################################

def ask_known2_view36(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = ScteKelvin2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_36')
    else:
        form = ScteKelvin2(excluded_properties=excluded_properties)

    return render(request, 'nitrogenio/nkelvin-scte-2.html', {'form': form})

###############################################################################

def ask_known3_view36(request):
    if request.method == 'POST':
        form = ScteKelvin3(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_36')
    else:
        form = ScteKelvin3()

    return render(request, 'nitrogenio/nkelvin-scte-3.html', {'form': form})
...
def process_values_view36(request):
    ...
    try:
        ...
        if third_property_choice == 7:
            ...
            h = subs_cls(6, 1, 4, pressao, energia_int3)
            ...
            h = subs_cls(6, third_property_choice, 5, third_value_input, entropia_esp)
        ...
        if cruzou_domo:
            ...
            for _ in range(60):
                ...
                try:
                    if is_isobaric:
                        h_mid = subs_cls(6, 1, 4, fixo_val, val_mid)
                    else:
                        h_mid = subs_cls(6, 1, 5, val_mid, fixo_val)
            ...
            try:
                if is_isobaric:
                    h_front_calc = subs_cls(6, 1, 4, fixo_val, val_front)
                else:
                    h_front_calc = subs_cls(6, 1, 5, val_front, fixo_val)
        ...
        return render(request, 'nitrogenio/results-nitrogenio-scte-4.html', {
            'fase': fase,
