...
def homepage_view31(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view31(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_31')
    else:
        form = ScteR410A()

    return render(request, 'R134A/R134A-scte-1.html', {'form': form})

###############################################################################

def ask_known2_view31(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known3_31')
    else:
        form = ScteR410A2(excluded_properties=excluded_properties)

    return render(request, 'R134A/R134A-scte-2.html', {'form': form})

###############################################################################

def ask_known3_view31(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('process_values_31')
    else:
        form = ScteR410A3()

    return render(request, 'R134A/R134A-scte-3.html', {'form': form})
...
def process_values_view31(request):
    ...
    try:
        ...
        if third_property_choice == 7:
            ...
            h = subs_cls(5, 1, 4, pressao, energia_int3)
            ...
            h = subs_cls(5, third_property_choice, 5, third_value_input, entropia_esp)
        ...
        if cruzou_domo:
            ...
            for _ in range(60):
                ...
                try:
                    if is_isobaric:
                        h_mid = subs_cls(5, 1, 4, fixo_val, val_mid)
                    else:
                        h_mid = subs_cls(5, 1, 5, val_mid, fixo_val)
            ...
            try:
                if is_isobaric:
                    h_front_calc = subs_cls(5, 1, 4, fixo_val, val_front)
                else:
                    h_front_calc = subs_cls(5, 1, 5, val_front, fixo_val)
        ...
        return render(request, 'R134A/results-R134A-scte-4.html', {
            'fase': fase,
