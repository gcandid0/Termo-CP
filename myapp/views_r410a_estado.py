...
from .forms import PropertyFormR410AEst1, SecondPropertyFormR410AEst1
from django.core.exceptions import ValidationError
from . import tabelas_termoprop as tbs

def processos_view23(request):
    return render(request, 'processos5.html')

###############################################################################

def ask_known1_view23(request):
    if request.method == 'POST':
        form = PropertyFormR410AEst1(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known2_23')
    else:
        form = PropertyFormR410AEst1()

    return render(request, 'R410A/R410A-estado-1.html', {'form': form})

###############################################################################

def ask_known2_view23(request):
    if request.method == 'POST':
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormR410AEst1(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            property_choice = form.cleaned_data['property_choice']
            value_input = form.cleaned_data['value_input']
            request.session['second_property_choice'] = property_choice
            request.session['second_value_input'] = value_input
            return redirect('process_values_23')
    else:
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormR410AEst1(excluded_properties=excluded_properties)

    return render(request, 'R410A/R410A-estado-2.html', {'form': form})
...
def process_values_view23(request):
    ...
    try:
        ...
        return render(request, 'R410A/results-R410A-estado-3.html', {
            'fase': fase,
...
def error_value_view23(request):
