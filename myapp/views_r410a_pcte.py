...
from .forms import PcteR410A, PcteR410A2, PcteR410A3, PcteR410A4
...
def ask_known1_view25(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_25')
    else:
        form = PcteR410A()

    return render(request, 'R410A/R410A-pcte-1.html', {'form': form})

###############################################################################

def ask_known2_view25(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = PcteR410A2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_25')
    else:
        form = PcteR410A2(excluded_properties=excluded_properties)

    return render(request, 'R410A/R410A-pcte-2.html', {'form': form})

###############################################################################

def ask_known3_view25(request):
    if request.method == 'POST':
        form = PcteR410A3(request.POST)
        if form.is_valid():
            temp_value_input = form.cleaned_data['value_input']
            # Armazena o valor da terceira entrada (Temperatura da vizinhança)
            request.session['temp_value_input'] = temp_value_input
            return redirect('ask_known4_25')
    else:
        form = PcteR410A3()

    return render(request, 'R410A/R410A-pcte-3.html', {'form': form})


###############################################################################

def ask_known4_view25(request):
    if request.method == 'POST':
        form = PcteR410A4(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_25')
    else:
        form = PcteR410A4()

    return render(request, 'R410A/R410A-pcte-4.html', {'form': form})
...
def process_values_view25(request):
    ...
    try:
        ...
        if estados.lista_estados:
            ...
            h = subs_cls(4, property_choice, second_property_choice, value_input, second_value_input)
        ...
        if third_property_choice == 8:
            ...
            h = subs_cls(4, 1, 4, pressao, entalpia_esp3)
        else:

            h = subs_cls(4, 1, third_property_choice, pressao, third_value_input)
        ...
        # Utilização correta do estado final (2) em relação ao estado inicial para cálculo das variações
        if third_property_choice == 8:
        ...
        # Iterando pela Entalpia (h) para desenhar o caminho isobárico sem quebra ao cruzar as fases
        ...
        pontos_criticos = []
        try:
            # Substituição para a substância 4 (R-410a)
            h_sat_l = subs_cls(4, 1, 6, pressao, 0)
            h_sat_v = subs_cls(4, 1, 6, pressao, 100)
        ...
        # Processamento da curva: Forçando o índice 4 (Entalpia) para R-410a
        for val_atual in valores_iteracao:
            try:
                # O '4, 1, 4' significa: Substância 4 (R-410a), Prop Conhecida 1 (Pressão), Iterando Prop 4 (Entalpia)
                h_micro = subs_cls(4, 1, 4, pressao, val_atual)
        ...
        return render(request, 'R410A/results-R410A-pcte-5.html', {
            'fase': fase,
...
def error_value_view25(request):
