# -*- coding: utf-8 -*-
"""
Created on Tue Apr 14 24:14:36 2026
...
def ask_known1_view24(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_24')
    else:
        form = PropertyFormR410AEst1()

    return render(request, 'R410A/R410A-propriedade-1.html', {'form': form})

###############################################################################

def ask_known2_view24(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('process_values_24')
    else:
        excluded_properties = [int(request.session.get('property_choice', 0))]
        form = SecondPropertyFormR410AEst1(excluded_properties=excluded_properties)

    return render(request, 'R410A/R410A-propriedade-2.html', {'form': form})
...
def process_values_view24(request):
    ...
    try:
        ...
        return render(request, 'R410A/results-R410A-propriedade-3.html', {
            'fase': fase,
...
def error_value_view24(request):
