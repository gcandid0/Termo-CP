...
from .forms import ScteR410A, ScteR410A2, ScteR410A3
from django.core.exceptions import ValidationError
from . import tabelas_termoprop as tbs
from . import estados as std
import json
import math # Importação necessária para escala logarítmica
...
def homepage_view26(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view26(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('ask_known2_26')
    else:
        form = ScteR410A()

    return render(request, 'R410A/R410A-scte-1.html', {'form': form})

###############################################################################

def ask_known2_view26(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = ScteR410A2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_26')
    else:
        form = ScteR410A2(excluded_properties=excluded_properties)

    return render(request, 'R410A/R410A-scte-2.html', {'form': form})

###############################################################################

def ask_known3_view26(request):
    if request.method == 'POST':
        form = ScteR410A3(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_26')
    else:
        form = ScteR410A3()

    return render(request, 'R410A/R410A-scte-3.html', {'form': form})
...
class subs_cls:
    """SUBS"""
    '''
    Calcula e imprime o valor das propriedades de diferentes substâncias a partir do valor conhecido de duas propriedades.
    Valores dados pelas tabelas B.2 a B.7.
    '''
...
def process_values_view26(request):
    ...
    try:
        ...
        # O formulário ScteR410A3 fornece:
        ...
        h = subs_cls(4, third_property_choice, 5, third_value_input, entropia_esp)
        ...
        # ==========================================================
        # GERAÇÃO DE MICROPROCESSOS PARA O DIAGRAMA (VIEW 26)
        # ==========================================================
        pontos_grafico = request.session.get('pontos_grafico', [])

        prop_fixa_idx = 5 # Processo Isentrópico (s=const)
        val_fixo = entropia_esp
        
        # A variação ocorrerá na propriedade escolhida pelo usuário para o estado 2
        prop_var_idx = third_property_choice
        
        if prop_var_idx == 0: # Variando Temperatura
            val_inicial = temperatura
            val_final = temperatura2
        else:                 # Variando Pressão (fallback também)
            prop_var_idx = 1
            val_inicial = pressao
            val_final = pressao2
        ...
        if prop_var_idx in [1, 2] and val_inicial > 0 and val_final > 0:
            ...
            # Escala Linear (para Temperatura, que pode ter valores negativos inclusive)
        ...
        if val_final != val_inicial:
            for val_atual in valores_iteracao:
                ...
                try:
                    # R-410a usa opt=4 em subs_cls
                    h_micro = subs_cls(4, idx1, idx2, val1, val2)
                    ...
                    if v_micro != 0:
                        ...
                        t_plot = val_fixo if prop_fixa_idx == 0 else round(h_micro.results[2][0][3], 2)
        ...
        return render(request, 'R410A/results-R410A-scte-4.html', {
            'fase': fase,
