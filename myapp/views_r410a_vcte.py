...
from .forms import vcteR410A, vcteR410A2, vcteR410A3, vcteR410A4
...
def homepage_view27(request):
    return render(request, 'Inicio.html')

###############################################################################

def ask_known1_view27(request):
    estados.limpar_estados()  # limpa a lista
    if request.method == 'POST':
        form = vcteR410A(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known2_27')
    else:
        form = vcteR410A()

    return render(request, 'R410A/R410A-vcte-1.html', {'form': form})

###############################################################################

def ask_known2_view27(request):
    # Recupera as propriedades excluídas da sessão
    excluded_properties = request.session.get('excluded_properties', [])

    if request.method == 'POST':
        form = vcteR410A2(request.POST, excluded_properties=excluded_properties)
        if form.is_valid():
            ...
            return redirect('ask_known3_27')
    else:
        form = vcteR410A2(excluded_properties=excluded_properties)

    return render(request, 'R410A/R410A-vcte-2.html', {'form': form})

###############################################################################

def ask_known3_view27(request):
    if request.method == 'POST':
        form = vcteR410A3(request.POST)
        if form.is_valid():
            ...
            return redirect('ask_known4_27')
    else:
        form = vcteR410A3()

    return render(request, 'R410A/R410A-vcte-3.html', {'form': form})

###############################################################################

def ask_known4_view27(request):
    if request.method == 'POST':
        form = vcteR410A4(request.POST)
        if form.is_valid():
            ...
            return redirect('process_values_27')
    else:
        form = vcteR410A4()

    return render(request, 'R410A/R410A-vcte-4.html', {'form': form})
...
def process_values_view27(request):
    ...
    try:
        ...
        # O formulário vcteR410A4 fornece:
        ...
        h = subs_cls(4, third_property_choice, 2, third_value_input, volume_esp)
        ...
        return render(request, 'R410A/results-R410A-vcte-5.html', {
            'fase': fase,
        ...
        return render(request, 'error_type27.html', {'message': str(e)})
    except Exception:
